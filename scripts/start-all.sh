...
# Shopping Project - All Services Startup Script
# Bu script tüm servisleri paralel olarak başlatır
...
echo -e "${BLUE}=========================================${NC}"
echo -e "${BLUE}  Shopping Project - Starting Services  ${NC}"
echo -e "${BLUE}=========================================${NC}"
echo ""
...
echo -e "${GREEN}[1/5] API başlatılıyor (http://localhost:$API_PORT)...${NC}"
...
echo -e "${GREEN}[2/5] Gateway başlatılıyor (http://localhost:$GATEWAY_PORT)...${NC}"
cd "$PROJECT_ROOT/src/Gateway"
dotnet watch --urls "http://localhost:$GATEWAY_PORT" &
GATEWAY_PID=$!
echo "Gateway PID: $GATEWAY_PID"

# 3. Admin Web Başlatma (React + Vite)
echo -e "${GREEN}[3/5] Admin Web başlatılıyor (http://localhost:$ADMIN_PORT)...${NC}"
cd "$PROJECT_ROOT/src/Presentation/Admin"
pnpm dev --port $ADMIN_PORT &
ADMIN_PID=$!
echo "Admin PID: $ADMIN_PID"

# 4. Web (Customer) Başlatma (React + Vite)
echo -e "${GREEN}[4/5] Web başlatılıyor (http://localhost:$WEB_PORT)...${NC}"
cd "$PROJECT_ROOT/src/Presentation/Web"
pnpm dev --port $WEB_PORT &
WEB_PID=$!
echo "Web PID: $WEB_PID"

# 5. App (React Native) Başlatma
echo -e "${GREEN}[5/5] App (React Native) başlatılıyor...${NC}"
...
echo ""
echo "Durdurmak için: ./stop-services.sh"
echo ""
