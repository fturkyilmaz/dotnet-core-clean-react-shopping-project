...
public sealed class ProductImage : ValueObject
{
    ...
    public static Result<ProductImage> Create(string url)
    {
        if (string.IsNullOrWhiteSpace(url))
            return Result.Failure<ProductImage>("Product image URL cannot be empty", ErrorType.Validation);

        var trimmedUrl = url.Trim();

        if (trimmedUrl.Length > MaxUrlLength)
            return Result.Failure<ProductImage>($"Image URL cannot exceed {MaxUrlLength} characters", ErrorType.Validation);

        if (!IsValidUrl(trimmedUrl))
            return Result.Failure<ProductImage>("Invalid image URL format", ErrorType.Validation);

        if (!HasValidImageExtension(trimmedUrl))
            return Result.Failure<ProductImage>($"Image must be one of: {string.Join(", ", _allowedExtensions)}", ErrorType.Validation);
